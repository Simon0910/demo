...
import static com.tr.core.encrypt.utils.EncodingUtils.base64Decode;
...
public final class EncryptionUtils {
    ...
    private static Logger logger = LoggerFactory.getLogger(EncodingUtils.class);
    ...
    public static Certificate loadEncryptionCertificate(InputStream certificate) throws CertificateException,
            NoSuchProviderException, FileNotFoundException {
        CertificateFactory factory = CertificateFactory.getInstance("X.509", "SUN");
        return factory.generateCertificate(certificate);
    }
    ...
    public static PrivateKey loadDecryptionKey(InputStream keyFile) throws GeneralSecurityException, IOException {
        byte[] keyDataBytes = read(keyFile);
        String keyDataString = new String(keyDataBytes, StandardCharsets.UTF_8);
        return loadDecryptionKey(keyDataBytes, keyDataString);
    }
    ...
    private static PrivateKey loadDecryptionKey(byte[] keyDataBytes, String keyDataString) throws GeneralSecurityException, IOException {
        if (keyDataString.contains(PKCS_1_PEM_HEADER)) {
            logger.debug("key type: {}", PKCS_1_PEM_HEADER);
        ...
        if (keyDataString.contains(PKCS_8_PEM_HEADER)) {
            logger.debug("key type: {}", PKCS_8_PEM_HEADER);
        ...
        logger.debug("key default type: {}", PKCS_8_PEM_HEADER);
