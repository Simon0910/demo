...
public final class MyProvider extends Provider {
    ...
    /**
     * UNCOMMENT FOR TESTING AFTER YOU'VE REPLACED THE ABOVE CERT WITH YOUR OWN.
     */
    public static void main(String[] argv) {
        System.out.println("Integrity check returned... " + MyProvider.selfIntegrityChecking());
    }


    public MyProvider() {
        // First, register provider name, version and description.
        super("MyProvider", 1.0, "sample provider which supports nothing");

        // Set up the provider properties here, see
        // Appendix A and B of JCE "How to Implement a Provider" Guide.
        //
        AccessController.doPrivileged(new PrivilegedAction() {
            @Override
            public Object run() {
//                put("MessageDigest.MyDigest", "com.xxx.myprovider.mydigest");
                return null;
            }
        });
    ...
    public static final synchronized boolean selfIntegrityChecking() {
        ...
        URL providerURL = (URL) AccessController.doPrivileged(
                new PrivilegedAction() {
                    @Override
                    public Object run() {
                        CodeSource cs = MyProvider.class.getProtectionDomain().getCodeSource();
    ...
    static class JarVerifier {
        ...
        private JarFile retrieveJarFileFromURL(URL url) throws PrivilegedActionException, MalformedURLException {
            ...
            jarURL = url.getProtocol().equalsIgnoreCase("jar") ?
                    url :
